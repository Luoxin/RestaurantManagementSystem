...
<head>
  ...
  <title>个人中心 - 订单详情-member_order_detail.jsp</title>
  ...
  <ul class="nav nav-list">
   ...
   String userId=(String)session.getAttribute("userId");
	...
	if (rs_order.next()) {
		...
		//start_读取商户表
		shangJiaID = rs_order.getString("shangJiaID");
		PreparedStatement rstm_shangjia = conn.prepareStatement("select * from shangjialist where shangJiaID=?");
		rstm_shangjia.setString(1, shangJiaID);
		ResultSet rs_user = rstm_shangjia.executeQuery();
		if(rs_user.next()) {
			shangjiaName=rs_user.getString("shangjiaName");//下单用户名
		}
		rstm_shangjia.close();
		//end
	...
	/*System.out.println(shangjiaName);
...
<div class="content-inner profile-changepwd"> 
       <form name="orderform" class="form-horizontal" method="post" id="changepwd_form" action="#"> 
        ...
        <table class="table table-condensed"> 
         <tbody> 
          <tr> 
           <td class="col1"><span class="item-name">商家:</span> <%=shangjiaName %></td> 
           <td class="col2"><span class="item-name">下单时间:</span><span class="om_time" data-time="2015-03-09 23:03:20"><%=orderTime %></span></td> 
           <td class="col3"><span class="item-name">送餐电话:</span><%=phone %></td> 
          </tr> 
          <tr> 
