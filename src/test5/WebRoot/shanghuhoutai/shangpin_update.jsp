...
<html lang="zh">
 <head> 
  ...
  <title>商户后台管理 - 修改菜品信息-shanghu_update.jsp</title> 
...
<div class="main-content">
 <div class="content-header"> 
  ...
  <%
	...
	ResultSet caipinSet=stm.executeQuery("select * from caipinglist");
	while(caipinSet.next()) {
		//查找到菜单名
		if(caipingID.equals(caipinSet.getString("caipingID"))) {
			caipingName=caipinSet.getString("caipingName");
			break;
		}
	}
	//读取菜品列表
	Statement stm1=conn.createStatement();
	ResultSet caidanSet=stm.executeQuery("select * from caidanlist");
	while(caidanSet.next()) {
		if(shangJiaID.equals(caidanSet.getString("shangJiaID")) 
				&& caipingID.equals(caidanSet.getString("caipingID"))) {
			jiage=caidanSet.getString("jiage");
			youhujiage=caidanSet.getString("youhujiage");
		}
	}
 ...
 <div class="content-inner profile-changepwd"> 
       <form class="form-horizontal" method="post" id="changepwd_form" action="update.jsp?caipingID=<%=caipingID %>"> 
        <fieldset>
          
          <div class="control-group clear-fix">     
          <label class="control-label" for=""><span class="required">*</span>菜品名</label> 
          <div class="controls"> 
           <input name="caipingName" id="" value="<%=caipingName %>" />       
          </div>
          <!--end input--> 
         </div>
         <!--end clearfix-->
         ...
         <div class="control-group">     
          <label class="control-label" for=""><span class="required">*</span>价格</label> 
          <div class="controls"> 
           <input name="jiage" id="" value="<%=jiage %>" onkeyup="value=value.replace(/[^\d]/g,'')" /><span>元</span>
          </div>
          <!--end input--> 
         </div>
         <!--end clearfix-->
          ...
          <div class="controls"> 
           <input name="youhujiage" id="" value="<%=youhujiage %>" onkeyup="value=value.replace(/[^\d]/g,'')" /><span>元</span>
          </div>
          <!--end input--> 
         </div>
         <!--end clearfix-->
          ...
          <button type="submit" class="btn btn-yellow" id="_submit">修改</button>
          <%    
