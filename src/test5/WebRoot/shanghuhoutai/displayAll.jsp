...
<html lang="zh">
 <head> 
  ...
  <title>商户后台管理 - 查看菜单列表-displayAll.jsp</title> 
...
<div class="main-content">
 <div class="content-header"> 
  <h2>菜单列表</h2> 
	...
	<script>
		function updselect(bt) {
			document.all_form.action="shangpin_update.jsp?caipingID="+bt.value;
			document.all_form.submit();
		}
		function delselect(bt) {
			var message=confirm("确认删除"+bt.name+"?");
			if (message) {//确认删除
				document.all_form.action="delete.jsp?caipingID="+bt.value;
				document.all_form.submit();
			}
		}
	</script>
 ...
 <div class="content-inner profile-changepwd"> 
       <form name="all_form" class="form-horizontal" method="post" id="changepwd_form"> 
        <fieldset>
		 <%
    Connection conn = ConnectionPool.getInstance().getConnection();
    //读取菜单列表
	Statement stm_caidanlist = conn.createStatement();
	ResultSet rs_caidanlist = stm_caidanlist.executeQuery("select * from caidanlist");
	//读取菜品列表
	Statement stm_caipinglist = conn.createStatement();
	ResultSet rs_caipinglist = stm_caipinglist.executeQuery("select * from caipinglist");
	
	out.print("<table width='90%' border=1><tr><th>菜品名</th><th>价格<th>优惠价格</th><th>修改</th><th>删除</th></tr>");						
	while(rs_caidanlist.next()) {
		String id = rs_caidanlist.getString("shangJiaID");
		//判定商户ID是否相同
		if(id.equals(shangJiaID)) {			
		    //菜单表中的菜品ID
			String a_ID = rs_caidanlist.getString("caipingID");
			while(rs_caipinglist.next()) {		
				//菜品 表中的菜品ID
				String b_ID = rs_caipinglist.getString("caipingID");
				String caipinName = "";
				if(a_ID.equals(b_ID)) {	
					out.println("<tr align='center'>");
					//菜品名 便于确认对话框
					caipinName=rs_caipinglist.getString("caipingName");
					out.print("<td>"+caipinName+"</td>");
					out.print("<td>"+rs_caidanlist.getString("jiage")+"</td>");
					out.print("<td>"+rs_caidanlist.getString("youhujiage")+"</td>");
					out.println("<td><button value=\""+b_ID+"\" onclick=\"updselect(this)\" name=\""+caipinName+"\"><span>修改</span></button></td>");
					out.println("<td><button value=\""+b_ID+"\" onclick=\"delselect(this)\" name=\""+caipinName+"\"><span>删除</span></button></td>");
					out.println("</tr>");
					break;
